...
*** two servers
    pros: can just duplicate existing configuration and change config.  keeps
    setups completely independent so there's no mistaking.

    cons: slightly more complex to provision, a bit pricier (about $8/month).
    annoying to have to copy things to separate servers...
** DONE set up "elastic IP" address for experiments.leap-lab.org
   this allows for re-mapping the domain to a different server without having to
